...
class RVRtext{
   ...
   * @param {Boolean} [isLink=false] - whether the item is a link (meaning hover state and click animations will be applied)
   * @param {Boolean} callback - click callback
   * @param {Boolean} context - what `this` would mean in the callback
   * */
  static createBackgroundPlane({data,width,height,isLink=false,callback,context}={}){
   ...
   * @param {Number} x - x position of the label (calculated against background width)
   * @param {Number} y - y position of the label (calculated against background height)
   * @param {Boolean} [isActive=false] - whether the link is selected
   * */
  static createText({data,text,x,y,width,isActive=false}={}){
